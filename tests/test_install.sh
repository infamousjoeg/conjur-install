...
start_conjur () {
    ...
    until [ "$(docker inspect -f "{{.State.Status}}" conjur-install_conjur_1)" == "running" ]; do
        sleep 0.1;
    done;
}
...
conjur_createacct () {
    ...
    CONJUR_INFO=$(sudo docker exec -i conjur-install_conjur_1 conjurctl account create quick-start)
...
conjur_init () {
    ...
    set -x
    sudo docker exec -i conjur-install_client_1 conjur init -u conjur -a quick-start 
    set +x
}
...
conjur_authn () {
    ...
    set -x
    sudo docker exec -i conjur-install_client_1 conjur authn login -u admin <<< "${CONJUR_API_KEY}"
    set +x
}
...
report_info () {
    ...
    sudo docker ps
    echo -e "Interact with it via Conjur CLI on conjur-install_client_1: ${CYAN}sudo docker exec -it conjur-install_client_1 bash${NC}"
